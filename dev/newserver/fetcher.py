...
def fetch():
  while True:
    ...
    try:
      ...
      firstTime = False
      if lastuid > 0:
        emails = db.getEmails(email, version)
        append = True
      else:
        emails = []
        firstTime = True
      ...
      while loaded < total:
        ...
        ok, data = imap_conn.uid('fetch', str(uids[min(start, len(uids)-1)])+ ":" + str(uids[min(start+JUMP-1, len(uids)-1)]), '(UID X-GM-LABELS X-GM-THRID BODY.PEEK[HEADER.FIELDS (FROM TO CC Date)])')
        ...
        if len(emails) >= REFRESH_NETWORK or loaded >= total:
          if append:
            db.storeEmails(email, emails, version)
            append = False
          else:
            # store the file
            db.storeEmails(email, emails, version + 1)
